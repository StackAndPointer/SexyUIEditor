...
class PreviewCanvas(QWidget):
    ...
    def _draw_widget(self, painter: QPainter, widget_id: str):
        ...
        props = w.properties
        x = int(props.get("mX", 0))
        y = int(props.get("mY", 0))
        width = int(props.get("mWidth", 100))
        height = int(props.get("mHeight", 30))
                ...
                if px and not px.isNull():
                    ...
                    if stretch:
                        ...
                        if scale_x != 1.0 or scale_y != 1.0:
                            scaled = px.scaled(int(px.width() * scale_x),
                                              int(px.height() * scale_y),
                                              Qt.AspectRatioMode.IgnoreAspectRatio,
                                              Qt.TransformationMode.SmoothTransformation)
                            painter.drawPixmap(x, y, scaled)
                        else:
